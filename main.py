...
global mySQLQuery #глобальная переменная, SQLзапрос 

bot = telebot.TeleBot('5511019814:AAE6VhsF7g-tYmAuOJG7UWfTBCl-Rpll40o')
@bot.message_handler(commands = ["start"]) #начало бота, команда старт, ввиводит кнопку "Головне меню"
def start(message):
	markup = types.ReplyKeyboardMarkup(resize_keyboard=True) #прописываем свойства кнопки
	btn1 = types.KeyboardButton("Головне меню") #прописываем текст кнопки
	markup.add(btn1) #вызываем метод класса markup add и добовляем нашу кнопку
	msg = bot.send_message(message.chat.id,"Привіт👋", reply_markup = markup) #отправляем сообщение
